...
class ChatBox:
    ...
    def rename_chat_name(self, old_name: str, new_name: str):
        self.init_session()
        if old_name in st.session_state[self._session_key]:
            st.session_state[self._session_key][new_name] = st.session_state[self._session_key].pop(old_name)
            if self._chat_name == old_name:
    ...
    def del_chat_name(self, name: str):
        self.init_session()
        # print("del_chat_name", name)
        # print(st.session_state[self._session_key])
    ...
    def history(self) -> List:
        self.init_session()
        # print("history", self._chat_name)
        # print(st.session_state[self._session_key].get(self._chat_name, []))
    ...
    def to_json(
        ...
        data = self.to_dict()
        # remove on_submit callback
        # history -> [any keys] -> "role": "assistant", "metadata": "feedback_kwargs" -> "on_submit"
        def remove_on_submit(d):
